...
Expected after seam patch 11: exactly EIGHTEEN files — the three above with six
...
`components/sessions/session-conversation-diff-panel.tsx`, seam patch 8's
one new one: `hooks/use-chat-landing-file-draft.ts`, which also adds hunks to
`session-chat-input-area.tsx`, seam patch 9's one: `components/session-list.tsx`,
seam patch 10's one: `components/sessions/session-side-panel-tab-bar.tsx`, which
also adds hunks to `session-detail.tsx`, and seam patch 11's two:
`components/sessions/session-browser-panel.tsx` and
`components/sessions/managed-preview-surface.tsx`, which also adds hunks to
`session-detail.tsx`. (This paragraph said FOURTEEN through seam patch 9, which
had added `session-list.tsx` without counting it; corrected here, not grown.)
...
### 10. Host-contributed side-panel tabs, driven and reported from outside (2026-09-02)
...
lines are named in `lody-surface-tabs.test.tsx`'s anchor table; the rest add
...
`packages/webapp/test/lody-surface-tabs.test.tsx` pins both files against the
...
### 11. A host-resolved viewer URL for the Browser panel (2026-09-02)
...
| 2 | 666, 672 | `onMobileBack,` and `onMobileBack?: () => void;` — the same anchor as seam patch 10 hunk 6 | declares and destructures `resolveManagedPreviewViewerUrl` |
...
and 15 takes the branch it took before. `packages/webapp/test/lody-surface-tabs.test.tsx`
