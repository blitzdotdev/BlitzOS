...
export type MobileHomeScreenProps = {
  ...
  /** Shows the GitHub segment of the Projects sub-tab selector. On by
     default. A platform without GitHub integration has no repositories to
     list, so the segment leads to an empty state and nothing else; with this
     off the segment is not rendered and a stale `'github'` sub-tab falls back
     to Local, the same rule `showTasksTab` applies. */
  showGitHubProjects?: boolean;
  /** Hides the first-run onboarding takeover on an empty Chat tab. It tells
     the reader to download and start Lody on their own computer, which is the
     mobile counterpart of the desktop hint band's `download-client` state; a
     host that already runs Lody for the member should not draw it. */
...
function ProjectsSubTabSelector({
  ...
  iosTheme: boolean;
  /** See `MobileHomeScreenProps.showGitHubProjects`. */
  showGitHub?: boolean;
}) {
...
function ProjectsTabView({
  ...
  /* A stored `'github'` choice must not survive the segment that sets it, or
     the tab opens on a list the platform cannot fill. */
  const effectiveSubTab: MobileProjectsSubTab = showGitHub ? selectedSubTab : 'local';
