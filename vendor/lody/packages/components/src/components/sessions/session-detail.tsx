...
/** The id of a HOST-contributed side-panel tab (seam patch 10). The prefix is
...
const SessionDetail = ({
  ...
  // Seam patch 10: the rule the two effects above apply to PR and Browser,
  ...
  const sidePanelFixedOptions = useMemo<(SessionSidePanelOption & { id: SidebarTab })[]>(() => {
    ...
    // Seam patch 10: host tabs are fixed panels for every purpose but
  ...
  // Seam patch 10: the host drives the panel the way its own `+` menu does.
  ...
  useEffect(() => {
    writeStoredLastActiveTabState(sessionId, {
      ...
      sidePanel: {
        open: isSidebarOpen,
        // Seam patch 10: a host tab is the host's to restore, and the stored
  ...
  // Seam patch 10: report the panel to the host. The callback is read through
  ...
  // Seam patch 10: the host tab's body, mounted only while it is the active one.
