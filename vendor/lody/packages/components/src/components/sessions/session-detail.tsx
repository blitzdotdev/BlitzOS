...
/** The id of a HOST-contributed side-panel tab (seam patch 23). The prefix is
...
const SessionDetail = ({
  ...
  /**
   * Offer Side Chat only once there is an assistant turn to fork from.
   *
   * The launcher forks the active conversation, and a fork needs a completed
   * assistant turn: without one `forkActiveConversation` returns after a
   * `toast.error`, so in a session that has not answered yet the entry looks
   * enabled, does nothing visible, and reads as broken. With this on it takes
   * the `disabled` state the launcher already has for an offline machine.
   *
   * Off by default, so a host that does not pass it gets exactly today's
  ...
  // Seam patch 23: the rule the two effects above apply to PR and Browser,
  ...
  const sidePanelFixedOptions = useMemo<(SessionSidePanelOption & { id: SidebarTab })[]>(() => {
    ...
    // Seam patch 23: host tabs are fixed panels for every purpose but
  ...
  const sideChatOption = useMemo<SessionSidePanelOption | null>(() => {
    ...
    return {
      ...
      // and a launch already in flight. Opt-in, so the default is upstream's.
      disabled:
  ...
  // Seam patch 23: the host drives the panel the way its own `+` menu does.
  ...
  useEffect(() => {
    ...
    writeStoredLastActiveTabState(sessionId, {
      ...
      sidePanel: {
        open: isSidebarOpen,
        // Seam patch 23: a host tab is the host's to restore, and the stored
  ...
  // Seam patch 23: report the panel to the host. The callback is read through
  ...
  // Seam patch 23: the host tab's body, mounted only while it is the active one.
