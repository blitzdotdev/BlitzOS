...
type TaskListDemoProps = TaskListProps & {
...
function TaskListDemo({
  ...
  useEffect(() => {
    setSelectedId(selectedTaskId ?? null);
  }, [selectedTaskId]);
  ...
  const archiveTask = (taskId: string) => {
    setTaskState((prev) => prev.filter((task) => task.taskId !== taskId));
  ...
  return (
    <div className={frameClassName}>
      <TaskList
        ...
        getTaskHref={(taskId) => `/demo/sessions/${taskId}`}
      />
    </div>
  );
}
...
const DEFAULT_ARGS: TaskListProps = {
...
export const EmptyRepo: Story = {
  args: {
    ...
    tasks: DEFAULT_ARGS.tasks.filter((task) => task.repoFullName !== 'loro-dev/empty-repo'),
  },
  render: (args) => <TaskListDemo {...args} />,
};
...
export const LongBranchName: Story = {
  args: {
    ...DEFAULT_ARGS,
    selectedTaskId: 'task-long-branch',
    repos: [{ repoFullName: 'loro-dev/loro', collapsed: false }],
    tasks: [
      {
        taskId: 'task-long-branch',
...
function buildLongListTasks(): TaskListProps {
  ...
  const tasks = Array.from({ length: 120 }, (_, index) => {
    ...
    return {
      ...
      isWaitingPermission: false,
    } satisfies TaskListProps['tasks'][number];
  });

  const chats = Array.from({ length: 18 }, (_, index) => ({
    taskId: `chat-long-${index}`,
    title: `Chat ${index + 1}`,
    repoFullName: null,
    branchName: '',
    ...
    isOffline: false,
    isWaitingPermission: false,
  })) satisfies TaskListProps['tasks'];

  return {
    selectedTaskId: 'task-long-0',
    repos,
    tasks: [...tasks, ...chats],
  };
}
...
const PR_STATUS_ARGS: TaskListProps = {
...
type StoryTask = TaskListProps['tasks'][number];
...
function buildStateTask(
  ...
  return {
    ...
    title,
    repoFullName,
    branchName: repoFullName ? `storybook/${taskId}` : '',
...
const ALL_DIFF_AND_PR_STATES_ARGS: TaskListProps = {
  selectedTaskId: 'state-open-mixed',
  repos: [
    { repoFullName: 'loro-dev/lody', collapsed: false },
    { repoFullName: 'loro-dev/loro', collapsed: false },
