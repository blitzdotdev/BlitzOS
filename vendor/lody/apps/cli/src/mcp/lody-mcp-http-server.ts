...
export async function startLodyMcpHttpServer(options: { logger: Logger }): Promise<void> {
  ...
  const builtinDisabled = process.env.LODY_MCP_BUILTIN_DISABLED === '1';
  if (builtinDisabled) {
    options.logger.info(
  ...
  if (process.env.LODY_MCP_HTTP_DISABLED === '1') {
    options.logger.info(
      builtinDisabled
        ? '[mcp-http] disabled via LODY_MCP_HTTP_DISABLED; built-in MCP remains disabled'
        : '[mcp-http] disabled via LODY_MCP_HTTP_DISABLED; using stdio MCP only'
    );
    return;
  }
