...
async function runStep(id, name, action) {
  ...
  const record = { id, name, ok: outcome.ok, reason };
  results.push(record);
  console.log(`${outcome.ok ? "PASS" : "FAIL"} ${id} ${name}: ${reason}`);
...
try {
  ...
  const ready = await runStep("s5", "ready", async () => {
    ...
    while (Date.now() <= deadline) {
      ...
      if (lastPhase === "ready") {
        ...
        return pass(`workspace=${workspaceId}; phase=ready; ssh metadata captured`, { ssh });
      }
      if (lastPhase === "error") {
    ...
    return fail(`workspace=${workspaceId}; timed out after ${readyTimeoutMs / 1_000}s; last phase=${lastPhase}`);
  });
  ...
  await runStep("s9", "credentials", async () => {
    if (sshInfo === null || keyPath === null) return fail("ready SSH metadata prerequisite failed");
    const adminInfo = { ...sshInfo, port: 2222 };
    const adminKnownHostsPath = join(workDir, "known_hosts_admin");
    const remoteCommand =
      "sudo docker exec blitz-box sh -c 'test -s /var/lib/blitz/box-credential.json && test -s /var/lib/blitz/origin'";
    const deadline = Date.now() + 90_000;
    let lastFailure = "not attempted";
    ...
    do {
      const scan = spawnSync(
        "ssh-keyscan",
        ["-T", "10", "-p", "2222", sshInfo.host],
        { encoding: "utf8", timeout: 15_000 },
      );
      if (scan.status === 0 && scan.stdout.includes(" ssh-")) {
        writeFileSync(adminKnownHostsPath, scan.stdout, { mode: 0o600 });
        const attempt = spawnSync(
          "ssh",
          sshArgs(adminInfo, adminKnownHostsPath, remoteCommand),
          { encoding: "utf8", timeout: 20_000 },
        );
        if (attempt.status === 0) {
          return pass("host admin SSH on port 2222 found non-empty box credential and origin");
        }
        lastFailure = commandFailure(attempt);
      } else {
        lastFailure = `host ssh-keyscan failed: ${commandFailure(scan)}`;
      }
    ...
    return fail(`credentials were not verifiable through host port 2222: ${lastFailure}`);
  });
} finally {
  stopSurfaceTunnel();
