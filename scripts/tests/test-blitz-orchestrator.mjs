...
console.log('orchestratorBootTask():')
{
  ...
  ok('mentions blitzscript workflows', /blitzscript/.test(duty))
  ok('carries the absolute blitzscript/llm.mjs import path', /src\/main\/blitzscript\/llm\.mjs/.test(duty), duty.slice(0, 120))
...
console.log('\nprepareAgentLaunch bootstrap injection (HOME sandboxed):')
{
  ...
  ok('orchestrators agent bootstrap carries the llm.mjs path', /blitzscript\/llm\.mjs/.test(boot7))
