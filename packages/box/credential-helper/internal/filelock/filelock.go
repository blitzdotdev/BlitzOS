...
// user of the control-plane credential is a separate short-lived process, and
// a sync.Mutex in one of them says nothing to the others.
package filelock
...
func chownToDirectoryOwner(lock *os.File, path string) error {
	...
	// systems. Refuse the chown if that invariant does not hold.
