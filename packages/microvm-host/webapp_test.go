...
const webAppTestToken = "01234567890123456789012345678901"

func webAppTestHandler(t *testing.T, backendURL string) http.Handler {
	t.Helper()
	dir := t.TempDir()
	...
	manager.vms["vm-1-webApp"] = &VM{
		VMID: "vm-1-webApp", GuestIP: "172.30.21.2", Status: StatusRunning,
	}
	backend, err := url.Parse(backendURL)
	if err != nil {
		t.Fatal(err)
	}
	api := NewHandler(manager, webAppTestToken).(*API)
	api.webAppTransport = &http.Transport{
...
func TestWebAppRoutingRestrictionAuthAndUnknownVM(t *testing.T) {
	...
	defer backend.Close()
	handler := webAppTestHandler(t, backend.URL)

	t.Run("requires bearer auth", func(t *testing.T) {
		request := httptest.NewRequest(http.MethodGet, "/vms/vm-1-webApp/webapp/7445/ports", nil)
	...
	t.Run("rejects every port outside the allowlist", func(t *testing.T) {
		for _, port := range []string{"22", "7443", "7446", "not-a-port"} {
			request := httptest.NewRequest(http.MethodGet, "/vms/vm-1-webApp/webapp/"+port+"/", nil)
	...
	t.Run("returns 404 for an unknown VM", func(t *testing.T) {
		request := httptest.NewRequest(http.MethodGet, "/vms/vm-9-missing/webapp/7445/", nil)
		request.Header.Set("Authorization", "Bearer "+webAppTestToken)
		response := httptest.NewRecorder()
		handler.ServeHTTP(response, request)
	...
	t.Run("proxies any HTTP method and strips its route prefix", func(t *testing.T) {
		request := httptest.NewRequest(
			http.MethodPatch,
			"/vms/vm-1-webApp/webapp/7445/workspace/a%20b?arg=one&arg=two",
...
func TestWebAppWebSocketBidirectionalPipe(t *testing.T) {
	...
	agentServer := httptest.NewServer(webAppTestHandler(t, backend.URL))
...
func TestMicroVMInitBridgesOnlyHostTapWebAppTraffic(t *testing.T) {
	...
	for _, required := range []string{
		...
		"for webapp_port in 7444 7445",
		"-i eth0 -p tcp -s \"$guest_gw\" -d \"$guest_ip\" --dport \"$webapp_port\"",
		"--to-destination \"127.0.0.1:$webapp_port\"",
	} {
		if !strings.Contains(script, required) {
			t.Fatalf("guest webApp bridge is missing %q", required)
		}
	}
}
