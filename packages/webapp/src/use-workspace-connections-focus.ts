...
import {
  ...
  type ConnectionsFocusResult,
} from './connections-focus';

/** Polls the box's `/connections-focus` marker on the ports cadence, while
 * the tab is visible, and calls `onFocus` at most once per focus the in-box
 * agent raises with `blitz connections open` (newest `requestedAt` wins).
 *
 ...
 * may set that baseline. The baseline is ephemeral (a ref), never persisted
 * to webApp-state. Mirrors use-workspace-preview-focus. */
export function useWorkspaceConnectionsFocus(
  enabled: boolean,
  workspaceId: string,
  filesBase: string | null,
  onFocus: (focus: ConnectionsFocus) => void,
): void {
  ...
  useEffect(() => {
    ...
    const consume = (result: ConnectionsFocusResult) => {
      ...
      if (consumedRef.current.workspaceId !== workspaceId) {
        // First successful observation after entering this workspace: adopt
        // the box's current focus as already-consumed and do not auto-open it.
        consumedRef.current = {
          workspaceId,
    ...
    const poll = async () => {
      ...
      const result = await fetchWorkspaceConnectionsFocus(filesBase, fetch, current.signal);
