...
import { PORTS_POLL_INTERVAL_MS, type ConnectionsFocus } from "../src/connections-focus.js";
import { useWorkspaceConnectionsFocus } from "../src/use-workspace-connections-focus.js";
import { render } from "./dom.js";

const FILES_BASE = "https://box.example/workspace/";

function marker(requestedAt: number, provider = "github"): ConnectionsFocus {
  return { version: 1, provider, requestedAt };
}

function focusResponse(focus: ConnectionsFocus | null): Response {
...
function Poller({ onFocus }: { onFocus: (focus: ConnectionsFocus) => void }) {
  useWorkspaceConnectionsFocus(true, "workspace-1", FILES_BASE, onFocus);
  return null;
}
...
describe("useWorkspaceConnectionsFocus", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
  ...
  // only be adopted from a poll that actually reached the box — the same
  // rules the preview-focus poller pins.
  it("does not adopt a baseline from a failed poll", async () => {
    ...
    const fresh = marker(1_787_000_005_000, "linear");
