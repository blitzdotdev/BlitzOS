...
/** Recorded provider answers. They used to ride the manifests, which meant the
 * shipped worker carried every recorded vendor body into production for the
 * sake of a test. They live here instead: imported by the conformance suite and
 * by nothing under `core/`, so `worker-source.mjs` cannot bundle them. */
...
/** A recorded token exchange. The exchange code is only ever proven against
 * these, never against a live provider, so an OAuth provider without one is a
 * conformance failure. */
...
/** One cheap authenticated call per provider, replayed status by status. Every
 * catalog entry needs an entry here; the suite pins the roster. */
export const PROBE_FIXTURES: Readonly<Record<string, readonly ProbeFixture[]>> = {
  "github": [
    {
