  ...
  const runtimeFor = (context: CoreContext): CoreRuntime => {
    ...
    return {
      ...
      vars: {
        ...
        boxPayloadRef: (context.env as TestBindings).BOX_PAYLOAD_REF,
        boxPayloadVersion: (context.env as TestBindings).BOX_PAYLOAD_VERSION,
...
export async function appRequest(
  ...
  return app.request(`https://cp.example${path}`, init, {
    ...
    BOX_PAYLOAD_REF: env.BOX_PAYLOAD_REF,
    BOX_PAYLOAD_VERSION: env.BOX_PAYLOAD_VERSION,
