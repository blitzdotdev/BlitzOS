...
export const discordManifest = {
  ...
  surfaces: {
    ...
    // No grant path (no OAuth, no pasted personal token) means no mint ever
    // renders a skill: the admin root mints through the connection row's own
    // placements in the static minter.
    skill: null,
  },
  probe: {
    request: (input) => ({
