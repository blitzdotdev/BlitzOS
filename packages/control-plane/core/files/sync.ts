...
async function runAttachments(
  ...
  return (await runAttachmentPass(runtime, attachments)).result;
}
...
async function convergeFilesReady(
  runtime: CoreRuntime,
  workspaceId: string,
...
export async function runFileSyncSweep(runtime: CoreRuntime): Promise<FileSyncResult> {
  ...
  for (const workspaceId of new Set(attachments.map(({ workspace_id }) => workspace_id))) {
    await convergeFilesReady(runtime, workspaceId, pass.syncedByWorkspace.get(workspaceId) ?? 0);
  }
...
export async function runReadyWorkspaceFileSync(
  ...
  let complete = await convergeFilesReady(
    runtime,
    workspaceId,
  ...
  for (const delayMs of retryDelaysMs) {
    ...
    complete = await convergeFilesReady(
      runtime,
      workspaceId,
      pass.syncedByWorkspace.get(workspaceId) ?? 0,
    );
