...
import { observeVisualViewportGeometry } from './mobile-cockpit';
...
import {
  ...
  TERMINAL_TOUCH_MOVE_THRESHOLD_PX,
...
const KEYBOARD_VIEWPORT_THRESHOLD_PX = 90;
  ...
  useEffect(() => {
    ...
    const stopObservingKeyboardGeometry = observeVisualViewportGeometry((geometry) => {
      if (
        ...
        keyboardBaselineHeight = geometry.height;
      } else {
        keyboardBaselineHeight = Math.max(keyboardBaselineHeight, geometry.height);
      }
      const nextKeyboardUp = (
        keyboardBaselineHeight - geometry.height > KEYBOARD_VIEWPORT_THRESHOLD_PX
      );
      if (keyboardUp && !nextKeyboardUp) {
        if (!keyboardHideRequested) setTextareaInputMode('none');
        keyboardHideRequested = false;
      }
    ...
    const pasteFromClipboard = async () => {
      ...
      try {
        ...
        if (!disposed) showPasteFailureHint();
      }
    };
    ...
    const terminalCellAtPoint = (clientX: number, clientY: number): TerminalCell | null => {
      ...
      return {
        col,
        row: Math.min(buffer.length - 1, buffer.viewportY + viewportRow),
      };
    };
    const showSelectionChipAt = (clientX: number, clientY: number) => {
      ...
      setSelectionChip({
        visible: true,
        x: Math.max(72, Math.min(rect.width - 72, clientX - rect.left)),
        y: Math.max(4, Math.min(rect.height - 38, clientY - rect.top - 42)),
      });
    };
    ...
    const selectionContainsCell = (cell: TerminalCell): boolean => {
      ...
      const start = selection.start.row * terminal.cols + selection.start.col;
      const point = cell.row * terminal.cols + cell.col;
    ...
    const handleCompatibilityMouseTail = (event: MouseEvent) => {
      const recentTouch = Date.now() - lastTouchAt < 1_500;
      const recentPromptTap = lastPromptTapAt > 0 && Date.now() - lastPromptTapAt < 1_500;
      if (!recentTouch || (!touchSelection && !recentPromptTap)) return;
      event.preventDefault();
      event.stopImmediatePropagation();
    };
