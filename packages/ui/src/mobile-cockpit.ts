...
export const MOBILE_COCKPIT_QUERY = '(max-width: 899px)';

export function useMobileCockpit(): boolean {
  const [mobile, setMobile] = useState(() => window.matchMedia(MOBILE_COCKPIT_QUERY).matches);
