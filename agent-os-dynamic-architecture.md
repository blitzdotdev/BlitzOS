...
5. **First milestone = the end-to-end vertical slice** (the §4 loop on ONE real scenario: a moment → suggested-reply widget → confirm → send). It reuses everything just merged and proves the whole thesis. BUT it acts on a logged-in account, so it **rides on the safety prerequisites**: **P0 (close the `/events` privacy leak) is the immediate first step**, then the consent/STOP gate, culminating in the slice (the visible payoff).
6. **BlitzOS is PURE SUBSTRATE; the connected agent makes ALL decisions (2026-06-05 directive).** BlitzOS provides perception (sensors → coalesced moments → `/events`), the tool/syscall surface, and transports — and **NO in-process decision logic**. There is no resident reasoner, no governor, no code that judges "significance" or chooses what to do. The agent (Claude over agent-socket, or a `claude -p` runner) IS the brain: it watches `/events`, decides what matters, and acts via the tools. *This supersedes the in-OS "resident brain" (L3) + "governor" (P2) below — they were built (P1) and then **removed**.* The only non-agent logic BlitzOS keeps is (a) **perception/coalescing** (scheduling raw signals into moments — "dumb but rich," not policy) and (b) **human-control safety** (consent gates, a STOP, dumb hard backstops) — never task/policy decisions. "BlitzOS should not try to be the agent."
...
### L3 — The brain = the CONNECTED AGENT (NOT in-OS code)

**Per §0 decision #6, BlitzOS ships no in-process brain.** The brain is the agent on the other end of the transport: a Claude over the agent-socket relay, or a `claude -p` runner on the localhost-bearer path. It consumes the moment stream (`/events`), decides — per moment — to ignore / perceive more / act, and acts through the existing tools. **All policy (significance, attention, what-to-do, budget) lives in the agent**, governed by its operating manual (`AGENTS_MD`/`tools.json`) — not in BlitzOS.

**Reuses:** the entire tool vocabulary as the agent's action surface (`create_surface`/`spawn_widget`/`surface_control`/`read_window`/`say`/the planned `focus`/`follow`); `/events` as its interrupt; `AGENTS_MD`/`tools.json` as its self-describing manual + stable prompt-cache blob; `~/.blitzos/session.json` (sessionFile.ts) for a local agent to auto-discover the localhost path.

**Explicitly NOT built (removed 2026-06-05):** a `brain/orchestrator` + `brain/reasoner` (an in-OS observe loop with code-decided significance/summaries) was built as P1 and then **deleted** — it was BlitzOS trying to be the agent. No `governor`, no attention-set/budget logic, no significance judgments in OS code. The agent self-governs (it manages its own budget/attention in its own context); BlitzOS's only non-agent logic is **perception/coalescing** (the `events.ts` scheduler — turning raw signals into moments, which is "dumb but rich," not policy) and **human-control safety** (consent gates + a STOP).

**Why the localhost-bearer path for a local runner:** it gives the agent scripted/structured reads the relay denies, *without* making `eval` routine (eval stays a localhost break-glass primitive, never the design center). A relay agent (pasted URL) is the zero-install path; both are just "the agent," and neither moves decision logic into BlitzOS.
...
### ~~P1 — in-OS resident loop~~ — BUILT then REMOVED (2026-06-05 directive)
A `brain/orchestrator` + `brain/reasoner` (an in-process observe loop with code-decided significance/summaries) was built, then **deleted** per §0 decision #6: BlitzOS must not be the agent. **The loop is run by the connected agent**, not by BlitzOS. There is nothing to build here — the agent (relay Claude or `claude -p`) already long-polls `/events` and decides. The substrate it needs (`/events`, the moment stream, the tools, `AGENTS_MD`) all exist.

### ~~P2 — Governor~~ — REMOVED (the agent self-governs)
No `brain/governor.ts`. Significance, attention, model-tiering, and budget are the **agent's** concern (in its own context/runtime), not BlitzOS code. What BlitzOS *does* keep, as **human-control safety** (not policy): the **consent gates** (P0 content-share + widget-data consent, already shipped), a **STOP / "take the wheel"** control to synchronously suspend agent action (TODO — must hard-abort in-flight CDP), and dumb hard backstops (rate-limit/size-cap already on the data route). These are guardrails the human controls, not decisions BlitzOS makes.
...
| The brain that initiates | the CONNECTED AGENT (relay Claude or `claude -p`) over `/events` + the full tool set; `AGENTS_MD`/tools.json as its manual | NONE in BlitzOS — the agent decides + self-governs (tiering/attention/budget live in the agent). Optional: a thin always-on `claude -p` runner carrying NO decision logic |
