...
const server = createServer(async (req, res) => {
  ...
  if (path === '/api/os/events' && req.method === 'GET') {
    ...
    res.write(
      `data: ${JSON.stringify({ type: 'hydrate', surfaces: wsHost.hydrateSurfaces(), camera: osState.camera || { x: 0, y: 0, scale: 1 }, mode: osState.mode || 'canvas', stageCount: osState.stageCount || 1, workspace: wsHost.active() })}\n\n`
    )
    sseClients.add(res)
