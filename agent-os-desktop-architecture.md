...
- CDP control surface per webview (`/windows/:id/control`) — **built (compile-verified)**: `click` (selector or x,y), `type` (Input.insertText), `eval` (Runtime.evaluate), `screenshot` (Page.captureScreenshot). Renderer reports each guest's webContents id on `dom-ready` (`os:register-webview`); main attaches `webContents.debugger` (`src/main/cdp.ts`). Behavioral verify pending `npm run dev` on Mac. Not yet: `wait-for`, per-key events, per-window consent scoping.
