...
function brainPrompt(url) {
  ...
  return [
    ...
    "On connect, FIRST assemble the desktop for this user as the manual's \"On connect\" section says (read list_state + list_integrations + your Notepad, then build a desktop tailored to their context). THEN run the /events long-poll loop the manual describes, FOREVER (never stop or exit on your own); prime `since` to the current `latest` before looping so you skip pre-startup history."
  ].join('\n')
}
