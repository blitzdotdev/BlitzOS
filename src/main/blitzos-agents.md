...
- **A window the user must TOUCH is staged alone, then sent back, never piled on the widget board.** The slot grid is for glanceable tiles; a full-size browser dropped on it buries every widget it covers. So when the user must sign into or drive web tools: use ONE tabbed browser (each tool a TAB, never a window per tool), keep it parked off-stage while you work, `bring_to_stage` only that one when they must act, and `send_backstage` it the instant they are done. Never leave more than one live browser on the board at a time. The five stacked sign-in windows that buried the case-file widgets are the exact failure to avoid: the right shape is one tabbed browser, staged alone, then parked.
