...
- **Read the STRUCTURE first — DOM for tabs, AX tree for windows. Screenshots are a last resort.** Default to `connection_read` / `connection_run_js` (a tab's DOM/text) and the AX tree (a window) to see and verify state. Do NOT take screenshots and reason over images unless the user explicitly asks for a screenshot, or the structure genuinely cannot be obtained (e.g. a canvas/native surface with no usable DOM/AX) — vision is the slowest path by far (a single image read is tens of seconds of model time), so reach for it only after the structured read has actually failed, not preemptively. Likewise prefer acting in the page (`run_js`/`act` by selector/ref) over coordinate clicking; coordinate clicks need the window raised (they steal the user's focus), so they too are a fallback, not the default.
- **Minimize round-trips — batch in one `connection_run_js`, go JS-first.** Every tool call is a full model turn (seconds), so the cost is the NUMBER of steps, not the work inside one. Do as much as possible per call: a single `connection_run_js` that reads, acts, AND returns a verification (e.g. set the title, then return the new title) beats a read → think → act → read loop. Banked/registry tools (above) collapse repeated work to one `connection_call_tool`.
...
For a task that is genuinely hard, large, massively parallel, adversarial, or over-context-window (mining many sessions, ranking N items, verifying every claim in a doc, deep research, a tournament, a wide migration), you can AUTHOR and RUN a workflow — a program you write that spawns local agent "leaves" over chunked work and aggregates their answers in code (the Claude Code workflow interface: `agent` / `parallel` / `pipeline` / `phase`). For a trivial or one-shot request, just answer directly.
- `start_workflow { task, title?, contextRefs? }` — spawn a fresh agent with the orchestrators capability ON and hand it a substantial task; it decides whether to write a workflow or do the task directly.
- `run_workflow { file, args?, title? }` — run a workflow `.js` you authored and `blitz check`ed. It returns immediately with a `runId`; the run continues in the background, reports its progress in chat as it goes, and writes its result to `<workspace>/.blitzos/workflows/<runId>/result.json` on completion.
- The full how-to (the `blitz` runner: `capabilities` → `check` → `run`, the injected globals, the act/ask boundary) is the orchestrator duty doc, handed to an agent with the orchestrators capability.
