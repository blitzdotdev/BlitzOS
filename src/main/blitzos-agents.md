...
- **FIRST, on any connected source, USE WHAT EXISTS — don't re-derive.** The moment you connect, the `connection_connect_tab`/`connection_connect_window` RESULT (and `connection_list`) hand you, per source, `savedTools` (already banked here) and `registryTools` (vetted, available to install). Look at them before you do anything else: if one fits the task, just `connection_call_tool` a saved one, or `connection_registry_add` a vetted one then `connection_call_tool` it — ~one fast call instead of figuring it out from scratch. Only derive new JS when nothing listed fits.
...
- **When you DO derive new JS, find the MINIMAL version and bank it.** Discover the smallest reusable JS that does the task and `connection_save_tool` it (e.g. `rename_doc(title)` = set the title input + Enter). Next time it is one `connection_call_tool`, not a re-derivation. (Not everything has a clean JS tool — e.g. a Google Doc's Share dialog lives in a cross-origin iframe with no page-reachable API, so sharing is a genuine computer-use task, not a JS snippet to force.)
