...
- **FIRST, on any connected source, USE WHAT EXISTS — don't re-derive.** `connection_list` already hands you, per source, `savedTools` (already banked here) and `registryTools` (vetted, available to install). If one fits the task, just `connection_call_tool` a saved one, or `connection_registry_add` a vetted one then `connection_call_tool` it — that's ~one fast call instead of figuring it out from scratch. Only derive new JS when nothing listed fits.
