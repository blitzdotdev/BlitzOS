...
app.whenReady().then(() => {
  ...
  // Per-agent reasoning-effort level (the island effort picker on a NEW chat): persist to meta + re-exec to apply.
  ipcMain.handle('os:agent-set-effort', (_e, p: { id: string; level: string }) => { try { return osSetAgentEffort(String(p?.id), String(p?.level)) } catch (e) { return { ok: false, error: (e as Error)?.message } } })
