...
| box-payload v1 | CP payload publisher + `core/box-config.ts` pin producer/result consumer ↔ box `blitz-payload` updater | `fixtures/box-payload/` | `test/box-payload-conformance.test.ts` (CP); `box/guest-tests/test/box-payload-conformance.test.ts` (arrives with the updater) |
