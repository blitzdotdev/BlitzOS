...
| preview-focus | `blitz preview open` CLI ↔ Go gateway (`/preview-focus`) ↔ browser (webApp consumer lands a later wave) | `fixtures/preview-focus/` | `box/actor/test/preview-focus-conformance.test.ts` (producer), `gateway/main_test.go` (reader) |
