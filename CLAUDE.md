...
- 7 files exceed the 700-line warn: `core/bootstrap.ts`, `core/compute/aws.ts`,
  ...
  the count is corrected here, not grown. `webapp/src/storage.ts` stayed under
  it 2026-08-24 by moving the shared-session decoders to
  `webapp/src/workspace-sessions.ts`.)
...
| org presence | CP `core/presence.ts` (PUT consumer, GET producer) ↔ browser `webapp/src/presence.ts` (`presenceViewForTabs` producer, `decodePresenceSnapshotResponse` consumer) | `fixtures/presence/` | `test/presence-conformance.test.ts` (CP), `webapp/test/presence-conformance.test.ts` (browser) |
| terminal session end | browser `webapp/src/terminal-session.ts` (`endWorkspaceTerminalSession` producer) ↔ Go gateway `box/gateway/main.go` (`POST /terminal/session/end`, maps kind→tmux prefix like `blitz-term`) | `fixtures/terminal-session-end/` | `box/gateway/main_test.go` (reader, fake killer), `webapp/test/terminal-session.test.ts` (producer); naming producer is `blitz-term` |
