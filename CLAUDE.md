...
identity. At HEAD only, `packages/box/Dockerfile` still installs the transition
`lody@0.88.1` npm artifact and applies five compiled-bundle scripts until plan
PR C; an upstream merge never selects or bumps that package.
...
- The five compiled-bundle scripts are transition behavior, not a merge
  procedure. They disappear only after plan PR C preserves or explicitly
  retires each behavior in the source-built image.
