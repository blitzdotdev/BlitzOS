...
| machine-stats | guest updater `rootfs/usr/local/libexec/blitz-payload` (one report per successful authenticated tick; normally every five minutes) ↔ CP consumer `core/machine-stats.ts` (`POST /workspaces/self/machine-stats`), which fills `machines.disk_used_percent` and surfaces as `MachineView.volumeUsedPercent` | `fixtures/machine-stats/` | `test/machine-stats-conformance.test.ts` (CP), `packages/box/guest-tests/test/machine-stats-conformance.test.ts` (guest: the real updater against a local origin) |
...
| box config v1 | CP `core/box-config.ts` produces `payload` and `features` (`GET /workspaces/self/box-config`) and consumes host results (`POST /workspaces/self/box-update-result`) ↔ host updater bash/python emitted by `core/bootstrap.ts` (`blitz-box-update`; cloud-VM path only) ↔ in-box `rootfs/usr/local/libexec/blitz-payload` consumes `payload` and `features` | `fixtures/box-config/` | `test/box-config-conformance.test.ts` (CP), `test/box-update-conformance.test.mjs` (real `python3` and `bash -n`), `test/box-update-host.test.mjs` (real bash and curl), `box/guest-tests/test/box-payload-conformance.test.ts` (real in-box parser) |
| box-payload v2 | `scripts/publish-box-payload.mjs` manifest and archive producer + `core/box-config.ts` pin/result side ↔ box `rootfs/usr/local/libexec/blitz-payload` updater and payload-owned s6 service tree | `fixtures/box-payload/` | `control-plane/test/box-payload-conformance.test.ts` (manifest + CP), `box/guest-tests/test/box-payload-conformance.test.ts` (real updater), `box/test/smoke.sh` (live E17–E20 add/remove/floor/features steps) |
