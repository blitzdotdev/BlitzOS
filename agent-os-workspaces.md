...
| **widget** | `*.widget/` bundle | `{ "props": {…}, "origin": "builtin:discord-servers" }` | **integration tokens** (Keychain); fetched data; **consent grant** (persisted, but only in agent-read-denied `.blitzos/state/consent.json` — never in the bundle or node entry, §9.3) |
...
3. **Content-share / consent PERSISTS (user decision, 2026-06-06), but only in agent-read-denied machine-local state.** The runtime authority stays the in-memory Set the renderer controls; on boot it is **hydrated from `.blitzos/state/consent.json`** (a `{ surfaceId(ULID) → true }` map). That file is gitignored, agent-read-denied (§9.4), and **never in a node entry, never in git, never portable** — so consent can't be forged by an agent file-write, can't travel with a copied/synced workspace, and never touches the P0 confused-deputy gate (srcdoc ids stay server-minted). Persistence requires the **stable surface ULIDs** from Phase 0 (a `srf-${zCounter}` id would re-key consent every session). Residual risk accepted by the user: a tab shared yesterday is still shared after a restart (vs. re-grant-each-session) — mitigated only by the share toggle staying one click to revoke. Lands in Phase 4.
...
**Phase 0 — stable ids.** Replace `srf-${zCounter}` identity with stable unique ids threaded through the node entry; `zCounter` becomes the session z-allocator only. No persistence yet. Smallest, safest unblocker. *(v1 impl: `crypto.randomUUID()` — already the codebase convention in `osActions`/`backend`, available in renderer + node, no new dep/file. ULID, the sortable variant the spec preferred, is a deferred drop-in if creation-ordered ids become useful.)* **DONE — `store.ts` renderer-created surfaces now mint stable ids; agent-created surfaces already did.**
...
4. **Does content-share consent ever persist?** ✅ **DECIDED (user, 2026-06-06): YES.** Consent survives restart, stored in agent-read-denied machine-local `.blitzos/state/consent.json` keyed by surface ULID — never in a node entry, never in git, never portable (§9.3). Depends on Phase 0 stable ids; implemented in Phase 4.
