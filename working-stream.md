...
1. **First-milestone arc = the end-to-end vertical slice** (decided): **P0 ✅ → P1 ✅ → P2 → P3.**
   - **P1 ✅ DONE — resident OBSERVE-ONLY loop.** `src/main/brain/{orchestrator.ts,reasoner.ts}`, started from `index.ts` after `startAgentSocket`. Consumes the in-process moment stream (`waitForEvents`, full content, trusted — no relay/redaction), reasons each significant moment into an Observation, logs to console + a 200-entry ring (inspect via localhost `GET /brain/log`). **Zero `osActions` import → cannot mutate.** Pluggable `Reasoner`: deterministic (default, zero-cost; significance = nav/idle/action) + optional headless `claude -p` (text-in/out, NO tools → observe-only by construction) via `BLITZ_BRAIN=claude`; `BLITZ_BRAIN=off` disables. Sidesteps the open "where the brain runs" §7 decision. Typecheck + build pass; deterministic reasoner unit-tested. Runtime = `npm run dev` (Mac).
   - **Next: P2** — governor (attention set, persisted budget, actions/min ceiling, degrade-to-observe) + consent/STOP gate + the Observe/Suggest/Act dial UI. Then **P3** = act tier (`focus`/`follow` os:action driving the built-but-unreachable `store.focusAndZoom`; new `op:'tool'` `send_reply` bridge path; agent-composed structured reads; suggested-reply context-widget).
