...
## 8. Auth OAuth flow has no app callback hook
- Constraint: the built-in Google OAuth flow owns callback routing and ends by minting a framework JWT session; an app cannot run its own logic inside the callback (invite redemption, stub-membership claim, one-shot bootstrap, membership binding) or choose the session outcome. The state cookie is an unsigned CSRF value with no app payload.
- Enforced: teenybase 0.0.15 auth module shape (audited 2026-08-16; see the teenybase auth audit table in [IDENTITY.md](IDENTITY.md)).
- Lift: an `onLogin(profile, ctx)` server hook that runs inside the callback with DB access — able to resolve/create the app's own user row, run side effects atomically, and either return claims for the framework session or take over the response and cookies. Plus a signed, app-extensible OAuth state payload (room for a PKCE verifier and app flags) verified constant-time.
- Until then: blitz-core hand-rolls the entire Google flow on D1 + Web Crypto in v2's shape (`packages/control-plane/core/identity/google.ts`, `oauth-state.ts`; shipped in identity phase 1).

## 9. Auth user records key on email/username, not provider subject
- Constraint: auth-table user records cannot key identity on the provider's stable subject (OIDC `sub`). Email is mutable and recyclable (Workspace admins rename and reassign addresses), so it is unsafe as the identity key.
- Enforced: teenybase 0.0.15 auth-table shape (same audit).
- Lift: a `(provider, provider_subject)` unique identity link with profile refresh on each login; email demoted to a verified, mutable attribute.
- Until then: blitz-core owns its `users` table keyed on `google_user_id` (`migrations/0009_identity.sql`).

## 10. No DB-backed opaque-token session option
- Constraint: sessions are stateless JWTs plus KV refresh rotation; there is no revocable server-row session and no app-owned session column (blitz needs `membership_id` on the session, rebindable for org switching).
- Enforced: teenybase 0.0.15 JWT/KV session model (same audit).
- Lift: an opt-in session mode — opaque random token in the cookie, SHA-256 hash in a D1 row, app-extensible columns, revocation by row delete, rebinding by row update.
- Until then: blitz-core keeps its own `sessions` table (hash-only, `membership_id`), minted from its own callback.

Deliberately not asked: generalizing membership/grant authorization into platform row rules. That authorization is relational and per-capability and stays application SQL (IDENTITY.md audit, last row) — recorded so nobody re-litigates it without new facts.
