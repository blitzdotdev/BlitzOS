...
- **`9d56259`/`f5bfcc4`/`c350e76` snappy onboarding** — reduced thinking effort (`--effort low` + `MAX_THINKING_TOKENS=2048`) restored on respawn when the duty's done; BRANCH_A embedded-chat layout; the director posts the opening question itself (~1.4s, was 50s+).
- **Conflict reconciliation (`buildBootstrap`):** adopted the branch's named-const-fragment refactor + duty-aware `guide` deferral, but the events fragments carry OUR **wait.sh** prose (`onConnect`/`waitLoop`/`keepChecking`), NOT the branch's pre-wait.sh raw `/events` poll text it carried forward (the silent-revert trap). Kept master's `writeWaitScript` + the `WAIT_SCRIPT` tail; took the branch's `established`/effort return. `claudeConversationExists`/`claudeEstablished` retained — dead code for agent `0`, LIVE for spawned agents (the jsonl backstop vs the restart crash-loop).
- **`workspace-host.mjs` auto-merged:** the branch's slot-aware `buildAgentSurface` (chat hydrates embedded) composes with master's runtime-surface re-assert (a re-asserted chat keeps its slot).
- **Verified:** typecheck · parity 11 · build green; `test-agent-fresh` **14/14** (primary `--session-id` never `--resume`, rotates fresh id, reduced effort under duty; spawned keep `--resume`); `test-runtime-surface-guard` holds; full unit suite green. **Live on the merged backend:** agent `0` respawned fresh (`--session-id`, meta id matches launched id), `wait.sh` written, chat round-trip replied, single process; `drive-{terminals,stages,newchat}` + `verify-real` PASS.
- **Open:** agent-0 boots orphan a stale `~/.claude/projects/<ws>/<uuid>.jsonl` each restart (branch TODO — harmless, no sweep yet). ⬜ Electron sandwich Mac pixel-test (no display here). ⬜ Push (5 ahead).
...
- **Follow-ups:** ✅ `set_theme` wired into the server transport (`15c46f8`) — `serverOps.setTheme` mirrors `osSetTheme`, broadcasts the `set-theme` action the shared renderer already applies; live-verified (valid hex → ok, invalid → graceful error). ✅ `drive-stages` step-1 de-flaked (poll-until-the-chat-renders; 2/2 clean). ⬜ Electron sandwich/`WebContentsView` needs a Mac pixel-test (no display here). ✅ re-merged the 4-commit delta `16953bb`→`c350e76` (the always-fresh primary + snappy onboarding) — see the `6697c90` section above.
...
## ✅ FIXED — runtime-surface-loss fragility (`77cd658`)
- **Was:** a renderer `sendState` (`os:state`) did a WHOLESALE `setState(s)` in the host's `onStatePush`, so a push that DROPPED a runtime surface (the chat widget — never serialized) deleted it from osState (gone until the next hydrate/switch), and a push that still carried a just-closed surface RESURRECTED it (why the junk cleanup needed a full restart — `close_surface` lost to the re-sync every round).
- **Fix** (the ONE shared seam both transports call — Electron `os:state`, server `/api/os/state` → `wsHost.onStatePush`): (1) RE-ASSERT host-owned runtime surfaces — rebuild any agent chat widget / activity panel a push dropped and add it back (cheap: id-compare first, only rebuild the usually-zero missing); (2) REJECT a stale re-push of an authoritatively-closed surface — `closeSurfaceFile` records the id ~8s, `onStatePush` filters those out.
- **Verified:** regression test `scripts/test-runtime-surface-guard.mjs` (re-assert when dropped, no-dup when present, closed surface not resurrected, fresh surfaces still apply); typecheck + parity + build + all host tests green; **live** — a hostile `/api/os/state` push omitting the chat returned ok but the chat SURVIVED; the leftover junk `close_surface` STUCK on the first try (was 4 rounds + never); chat survived a full drive-suite churn; drive-{terminals,stages,newchat} PASS on the new backend. Deployed (backend restarted).
- ✅ **Home test-junk cleared + restarted clean** (earlier this turn) — 13 junk panels + test agent "Chat 1" removed; primary `chat.md`/Notepad preserved; agent 0 `--resume`d + replied in 5s; its terminal auto-shows claude. From-scratch baseline verified.
