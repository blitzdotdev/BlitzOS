...
## ⚠️ Latent fragility found (NOT yet fixed — worth hardening)
- **Runtime surfaces can be clobbered out of osState.** During heavy live test churn (15+ reloads), the primary **chat WIDGET surface** ("chat") vanished from Home's osState — the chat DATA was safe (`chat.md` intact), but the runtime surface only rebuilds on the host's hydrate/switch, not a page reload, so it stayed gone. Recovered by a workspace switch round-trip (Home→other→Home → `performSwitch`→`buildAgentSurfaces()` rebuilds it). Root-cause likely a renderer state-push overwriting osState's runtime surfaces. Harden: the host should re-assert runtime surfaces (chat/activity) on every reconcile, never let a renderer push delete them. Also: live Home has accumulated test-junk srcdoc surfaces from drive runs — a "clear home board" sweep is due.
