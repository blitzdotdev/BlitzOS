...
- ⬜ **`drive-inbox` is not self-contained** — it only ships the "verify" half (expects pre-seeded `action-items.json` + a backend reload), so it fails standalone. Make it self-seed so the suite is green everytime.
- ⬜ **Stale plan docs** — `guardian-angel-blitzos.md`, `blitzos-visceral-loop.md`, `session-tape-and-daydreaming.md`, `onboarding-case-file.md`, `working-stream.md`, `multiple-workspace-areas*.md` still cite the deleted `agent-runner.mjs` / pre-rename "areas". CLAUDE.md + this tracker are corrected; the plan docs are backlog/aspirational (decide whether to refresh or mark historical).
