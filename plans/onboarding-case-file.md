...
- [ ] **Non-overlappable fixed widgets (macOS-desktop-style)** — replace free-floating board cards with a snapping, non-overlapping fixed grid (like macOS desktop widgets): cards tile, drag swaps/reflows instead of overlapping, no stacking. Replaces the hand-tuned slot coords + renderer clamp dance.
...
**The resident brain's first duty + the canned tier shipped.** The interview rides the EXISTING per-session brain machinery (`ensureBrain` → `startAgentRunner` → supervised `claude -p` with `--resume` continuity) — no second brain runtime was built:

- **Standing-duty seam** (`agent-runner.mjs` `getBootTask`, policy-free): the runner re-reads an optional duty string per spawn and injects it into the bootstrap prompt with an explicit license to act unprompted for its scope. Session '0' threads `interviewBootTask()` (index.ts) — returns the duty while `interview.json` is `pending`, drops off the next respawn once done.
