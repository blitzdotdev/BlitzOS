...
2. **The board assembles (deterministic — zero LLM dependency).** A fresh `Case File` workspace fills with library widgets animating in, seeded straight from scan JSON: project dossier grid (`widgets/dossiers.html` — already a scored-entity-card grid), cadence/timeline card, tooling card, and a "gaps" card with visible empty slots (what the OS *doesn't* know yet — the interview's table of contents).
...
- **Template widgets** (`widgets/`, manifest-registered): `profile`, `rhythm` (week×hour punchcard, cool→warm `heatLo→heatHi` ramp), `toolbox` (library-only — its board card was cut), `gaps`, `workflows` (favicon w/ brand-color letter fallback; Open → `blitz.tool open_window`; `connectable` badge).
