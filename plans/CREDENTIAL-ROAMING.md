...
- **Delivery, claude**: the PATH shim (mint → `CLAUDE_CODE_OAUTH_TOKEN` → exec pinned binary, failure non-fatal) for terminals; `options.env.CLAUDE_CODE_OAUTH_TOKEN` in the actor for chat; delete `managed-settings.json` at register; delete the `getOAuthToken` branch. One cheap harness run confirms the env var at the new pins (2.1.228) before relying on it.
