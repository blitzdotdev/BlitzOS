...
**Session arc (all on `master`, NOT pushed — user pushes, no SSH key here):** widget system (`52830bc`) → merged teammate autonomy kernel (`4781b47`) → dynamic-OS architecture doc (`8869776`) → P0 per-surface content-share privacy gate (`f306423`) → P1 resident brain (`8e9e576`, later NUKED) → P5 server-mode autonomy parity (`281eb21`) → server-mode nav-desync fix (`61b9d8a`) → in-canvas Chat + `say` (`c61cfca`) → **nuked the in-OS brain → pure substrate** (`1c1392c`) → professions catalog (`d995374`) → **agent-runner** (boots + auto-restarts the brain) (`59b2b84`+`b3b4bc9`+`1417546`) → **window-management perception + discipline** (`c0b90b0`). Demo self-supervises via `BLITZ_AGENT=claude bash preview/start-all.sh`.

**LATEST (`c0b90b0`):** `list_state` now returns the full layout — `viewport {w,h}`, `view {x,y,w,h,cx,cy,scale}` (the world-rect the user can SEE — fixes "windows land off-screen"), per-surface `z`, `mode` — and both AGENTS_MD + the brain prompt carry a **Window management** discipline (size for content+viewport, place INSIDE view, move/resize existing windows to make room, reflow survivors on close). Renderer `sendState` push throttles camera churn (250ms). Typecheck+build clean; needs a demo restart to go live.
