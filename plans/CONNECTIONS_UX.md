...
**No `manifest.json` exists.** Provider manifests are TS modules (`catalog/<provider>.ts`, `satisfies ProviderManifest`) because they hold hooks (probe builder) and multi-line skill templates, typecheck teaches agents at edit time, and the Worker bundles everything at build time anyway. JSON appears only as derived artifacts: the `workspaces.manifest` D1 column (the per-workspace ceiling — always say "workspace manifest" vs "provider manifest", they are different things), the mint-time-rendered `/var/lib/blitz/connections/mcp.json` placement, and the `GET /connections/catalog` response the picker reads.
