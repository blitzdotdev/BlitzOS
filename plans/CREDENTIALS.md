...
5. **Mode-(b) parity**: emit `core/credentials/`; set `CRED_MASTER_KEY` once via the platform secrets API before the commit; probe validation. (Blocked on the user-owned unlock: PR #11 deployed. The teenybase unlock is resolved — the repo now pins the published `teenybase@0.0.14`.)
